...
def load_model(
    ...
    classes = list(checkpoint_preview.get("classes", DEFAULT_CLASSES)) if isinstance(checkpoint_preview, dict) else list(DEFAULT_CLASSES)
    prompt_prototypes = (
        checkpoint_preview.get("prompt_prototypes", DEFAULT_PROMPT_PROTOTYPES)
        if isinstance(checkpoint_preview, dict)
        else DEFAULT_PROMPT_PROTOTYPES
    )
    ...
    checkpoint, missing_keys, unexpected_keys = load_checkpoint_flexible(model, checkpoint_path, device)
    if missing_keys:
...
def process_tasks(
    ...
    device = next(model.parameters()).device
    checkpoint_thresholds = checkpoint.get("val_thresholds", DEFAULT_THRESHOLDS) if isinstance(checkpoint, dict) else DEFAULT_THRESHOLDS
    checkpoint_postprocess = checkpoint.get("postprocess_cfg", DEFAULT_POSTPROCESS) if isinstance(checkpoint, dict) else DEFAULT_POSTPROCESS
    ...
    if isinstance(checkpoint, dict):
        for key in ["epoch", "run_name", "model_type", "val_thresholds"]:
    ...
    with open(output_path_obj, "w", encoding="utf-8") as file_obj:
        json.dump(predictions_payload, file_obj, ensure_ascii=False, indent=2)
    if save_debug_json:
    ...
    LOGGER.info("纯推理总耗时: %.2fs, 平均每张图: %.2fs", inference_total_time, avg_time_per_image)
