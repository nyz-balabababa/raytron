...
def _build_default_argv():
    argv = [
        ...
        "--resume_best", str(DEFAULT_RESUME_BEST),
        "--run_name", str(DEFAULT_RUN_NAME),
        "--text_cache_path", str(DEFAULT_TEXT_CACHE_PATH),
    ]
    if DEFAULT_RESUME is not None:
        argv.extend(["--resume", str(DEFAULT_RESUME)])
    if DEFAULT_NO_VAL:
        argv.append("--no_val")
    if DEFAULT_NO_TRAIN_SPLIT_FILTER:
        argv.append("--no_train_split_filter")
    if DEFAULT_NO_AUTO_RESUME:
        argv.append("--no_auto_resume")
    if DEFAULT_REBUILD_TEXT_CACHE:
        argv.append("--rebuild_text_cache")
    return argv
...
def build_argparser():
    ...
    parser.add_argument("--resume_best", type=Path, default=None)
...
def save_checkpoint(path, epoch, model, optimizer, scheduler, history, best_metrics, args):
    ...
    try:
        ...
        if not tmp_path.exists() or tmp_path.stat().st_size <= 0:
            raise RuntimeError(f"临时 checkpoint 写入失败或文件为空: {tmp_path}")
...
def resolve_resume_checkpoint(args, run_dir: Path):
    if args.resume is not None:
        manual_resume_path = Path(args.resume)
        if not manual_resume_path.exists():
            raise FileNotFoundError(f"--resume 指定文件不存在: {manual_resume_path}")
        if manual_resume_path.stat().st_size <= 0:
            raise RuntimeError(
                f"--resume 指定的 checkpoint 为空文件: {manual_resume_path}。"
                "请换成有效权重，或改用 --resume_best / --no_auto_resume。"
            )
        return manual_resume_path
    auto_resume_path = run_dir / "last.pt"
    if not args.no_auto_resume and auto_resume_path.exists():
        try:
            if auto_resume_path.stat().st_size <= 0:
                LOGGER.warning("检测到空的自动续训 checkpoint，已跳过: %s", auto_resume_path)
                return None
        except Exception as exc:
            LOGGER.warning("检查自动续训 checkpoint 失败，已跳过: %s error=%s", auto_resume_path, exc)
            return None
...
if __name__ == "__main__":
    ...
    try:
        sys.argv = _build_default_argv() + original_argv[1:]
        main()
    finally:
