...
from config_clipseg import (
    ...
    IMG_SIZE, BATCH, EPOCHS, PATIENCE, DEVICE, WORKERS, SEED,
    ...
    TINY_PROTECT_CLASSES, TINY_AREA_THRESHOLDS,
)
...
def _map_location(device):
    """torch.load 的 map_location 必须是 str，DEVICE=0 会报 'int' object is not callable."""
...
class CLIPSegDataset(Dataset):
    ...
    def __init__(
        ...
        # 用文件路径 + mtime + 大小做 hash，防止不同实验复用旧缓存
        pred_json = Path(pred_json)
        cache_key = stable_hash([
            str(pred_json.resolve()),
            pred_json.stat().st_mtime,
            pred_json.stat().st_size,
        ])
        cache_dir = PROJECT / ".mask_cache" / f"{pred_json.stem}_{cache_key[:8]}"
        ...
        if use_manifest:
            ...
            for record in tqdm(records, desc="  解码Manifest"):
                ...
                cache_name = stable_hash([img_path, prompt, grade, mask_source])
                ...
                self.samples.append((img_path, prompt, cache_path, sample_weight, True, meta))
...
def collate_fn(batch):
    ...
    masks = torch.stack([b[2] for b in batch], dim=0).squeeze(1)  # [B,1,H,W]→[B,H,W]
    valid_masks = torch.stack([b[3] for b in batch], dim=0).squeeze(1)
...
def train_epoch(model, processor, loader, optimizer, dice_loss, scheduler, epoch, total_epochs):
    ...
    for imgs, prompts, masks, valid_masks, _, _, weights, _ in pbar:
        imgs = imgs.to(DEVICE); masks = masks.to(DEVICE); valid_masks = valid_masks.to(DEVICE)
        ...
        # 只对正样本做 weight floor 保底；负样本保持原始 NEGATIVE_SAMPLE_WEIGHT
        is_positive_sample = (masks.flatten(1).sum(dim=1) > 0)
        weights = torch.where(
            is_positive_sample,
            weights.clamp(min=effective_weight_floor),
            weights,
        )
        if pbar.n == 0:
            pos_w = weights[is_positive_sample]
            neg_w = weights[~is_positive_sample]
            logger.info(
                f"  weight: pos_n={int(is_positive_sample.sum().item())}, "
                f"neg_n={int((~is_positive_sample).sum().item())}, "
                f"pos_min={float(pos_w.min().item()) if pos_w.numel() else -1:.3f}, "
                f"neg_mean={float(neg_w.mean().item()) if neg_w.numel() else -1:.3f}"
            )
        bce_map = F.binary_cross_entropy_with_logits(logits, masks, reduction="none")
        valid_area = valid_masks.sum(dim=(1, 2)).clamp(min=1.0)
        bce_per_sample = (bce_map * valid_masks).sum(dim=(1, 2)) / valid_area
...
def validate(model, processor, loader):
    ...
    vis_samples = []
    for imgs, prompts, masks, valid_masks, orig_prompts, _, _, _ in tqdm(loader, desc="  Val"):
...
def train():
    ...
    val_ds = CLIPSegDataset(
        ...
        augment_prompt=False,
        oversample=None,
        include_negatives=False,  # 合训版关闭 val 负样本，只看正样本分割
        negative_ratio=0.0,
        negative_weight=NEGATIVE_SAMPLE_WEIGHT,
        apply_score_filter=APPLY_SCORE_FILTER,
        use_manifest=False,
    ...
    # 断点续训
    start_epoch = 0; history = defaultdict(list); best_miou = -1; epochs_no_improve = 0
    resume_ckpt = None
    ...
    if resume_ckpt:
        ...
        ckpt = torch.load(resume_ckpt, map_location=_map_location(DEVICE), weights_only=False)
    ...
    for epoch in range(start_epoch + 1, EPOCHS + 1):
        ...
        if current_miou > best_miou:
            ...
            epochs_no_improve += 1
            logger.info(f"  mIoU 未提升 ({epochs_no_improve}/{PATIENCE})")
            if epochs_no_improve >= PATIENCE:
                logger.info(f"早停触发，best mIoU={best_miou:.4f}")
                break
...
def main():
    ...
    if USE_MANIFEST:
        ...
        logger.info("  Manifest weight floor: 0.1")
