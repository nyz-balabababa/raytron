#!/usr/bin/env python3
"""
YOLO-World + FastSAM 两阶段训练与评估 v3
==========================================
v2→v3 改动（基于 v1/v2 对比实验结论）:
  回退:
    - cls/box/label_smoothing → Ultralytics 默认 (v2 证明摧毁了同义词泛化)
    - computer 阈值 0.4→0.5, 过采样 5×→2×
    - tree 从多尺度移除 → 单尺度 (v2 多尺度降 5.7% mIoU)
  保留(v2 验证有效):
...
def plot_training_curves(results_csv, run_dir):
    ...
    plt.tight_layout()
    out_path = run_dir / "training_curves_v3.png"
...
def train():
    """阶段一：YOLO-World v3 检测训练（v3：默认 loss + 捕获指标 + 绘图）。"""
...
def main():
    ...
    logger.info("")  # 空行
    logger.info("=" * 60)
    logger.info("YOLO-World + FastSAM v3 两阶段训练与评估")
    logger.info(f"  v3 配置:")
    logger.info(f"    - 多尺度推理: {MULTI_SCALE_CLASSES} @ {MULTI_SCALES}")
    logger.info(f"    - cls/box/label_smoothing: Ultralytics 默认 (不回传给 model.train)")
    ...
    # ── 阶段二：FastSAM 两阶段 mask 评估（v3: person/animal 多尺度）──
