...
TRAIN_MODE_MANIFEST = USE_MANIFEST
TRAIN_MANIFEST_JSON = MANIFEST_JSON
TRAIN_PRED_JSON = PRED_JSON
TRAIN_VAL_PRED_JSON = VAL_PRED_JSON
TRAIN_REBUILD_DATASET = REBUILD_DATASET
TRAIN_RUN_NAME = RUN_NAME
TRAIN_YOLO_DIR = YOLO_DIR
TRAIN_DEVICE = DEVICE
...
def normalize_allowed_path(img_path, allow_set):
...
def convert_split_from_manifest(split, manifest_json, allow_set, images_dir, labels_dir, do_oversample=True):
    """
    manifest records -> bbox 标签（YOLO 检测格式: class_id cx cy w h）。
    ...
    - rle 非空
    - prompt 在 CLASSES
    - image_path 在当前 split 列表
    """
    logger.info(f"{'─' * 50}\n转换 {split} 集 (manifest): {manifest_json.name}")
    for d in [images_dir, labels_dir]:
        if d.exists():
            shutil.rmtree(d)
        d.mkdir(parents=True, exist_ok=True)
    ...
    for record in records:
        ...
        if not record.get("include_in_train"):
            filtered_counts["not_in_train"] += 1
            continue
        if not record.get("selected_hit"):
            filtered_counts["not_selected_hit"] += 1
            continue
        if record.get("rle") is None:
            filtered_counts["empty_rle"] += 1
            continue
        img_path = normalize_allowed_path(record.get("image_path", ""), allow_set)
        if img_path is None:
            filtered_counts["not_in_split"] += 1
            continue
        by_image[img_path][prompt].append(record)
        grade_counts[str(record.get("grade", "unknown"))] += 1
        source_counts[str(record.get("mask_source", "manifest"))] += 1
        if record.get("is_tiny"):
    ...
    pbar = tqdm(by_image.items(), desc=f"  转换{split}(manifest)", unit="img")
    for img_path, prompt_dict in pbar:
        label_lines = []
        has_animal = False
        ...
        for prompt in sorted(prompt_dict.keys(), key=lambda x: CLASSES.index(x)):
            class_id = CLASSES.index(prompt)
            for record in prompt_dict[prompt]:
                mask = rle_to_mask(record["rle"])
                bboxes = mask_to_bboxes(mask)
                if not bboxes:
                    prompts_filtered += 1
                    continue
    ...
    if do_oversample and animal_mult > 1 and animal_records:
        copy_idx = total_base
        for _, img_path, label_lines in animal_records:
            n = _copy_oversample(copy_idx, img_path, label_lines, images_dir, labels_dir, animal_mult - 1)
            copy_idx += n
            oversample_log["animal"] += n
        total_base = copy_idx
    ...
    logger.info(f"  图片: {total_unique} → {total_base} 张, 跳过空: {empty_skipped}")
    logger.info(f"  实例保留: {prompts_kept}, 过滤: {prompts_filtered}")
    logger.info(f"  Manifest grade 分布: {dict(sorted(grade_counts.items()))}")
    logger.info(f"  Manifest mask_source 分布: {dict(sorted(source_counts.items()))}")
    logger.info(f"  Manifest tiny 分布: {dict(sorted(tiny_counts.items()))}")
    if filtered_counts:
        logger.info(f"  Manifest 过滤: {dict(sorted(filtered_counts.items()))}")
    if oversample_log:
        logger.info(f"  过采样: animal={animal_mult}×(+{oversample_log['animal']})")
    return total_base
...
def align_imgsz(value, stride=32):
...
def _multi_scale_detect(yolo, img_rgb, prompt, base_imgsz, scales, conf, nms_iou, device):
    ...
    for scale in scales:
        imgsz = align_imgsz(int(base_imgsz * scale))
...
def plot_training_curves(results_csv, run_dir):
    ...
    plt.tight_layout()
    out_path = run_dir / "training_curves_world_new.png"
...
def train():
    ...
    device_str = f"cuda:{TRAIN_DEVICE}" if isinstance(TRAIN_DEVICE, int) and torch.cuda.is_available() else str(TRAIN_DEVICE)
    logger.info(f"模型: {YOLO_MODEL} task={YOLO_TASK} classes={CLASSES} device={device_str}")

    t0 = time.time()
    results = model.train(
        data=str(YOLO_DIR / "dataset.yaml"),
        epochs=EPOCHS, imgsz=IMG_SIZE, batch=BATCH,
        device=TRAIN_DEVICE, workers=WORKERS, seed=SEED,
...
def main():
    ...
    logger.info("")  # 空行
    logger.info("=" * 60)
    logger.info("YOLO-World + FastSAM manifest 两阶段训练与评估")
    logger.info("  配置:")
    logger.info(f"    - TrainMode: {'manifest' if TRAIN_MODE_MANIFEST else 'pred_json'}")
    logger.info(f"    - RebuildDataset: {TRAIN_REBUILD_DATASET}")
    logger.info(f"    - Device: cuda:{TRAIN_DEVICE}" if isinstance(TRAIN_DEVICE, int) else f"    - Device: {TRAIN_DEVICE}")
    logger.info(f"    - YOLO_DIR: {TRAIN_YOLO_DIR}")
    logger.info(f"    - RunName: {TRAIN_RUN_NAME}")
    if TRAIN_MODE_MANIFEST:
        logger.info(f"    - manifest: {TRAIN_MANIFEST_JSON}")
    else:
        logger.info(f"    - train 伪标签: {TRAIN_PRED_JSON}")
    logger.info(f"    - val   伪标签: {TRAIN_VAL_PRED_JSON}")
    logger.info(f"    - 多尺度推理: {MULTI_SCALE_CLASSES} @ {MULTI_SCALES}")
    logger.info(f"    - animal 过采样 {RARE_OVERSAMPLE.get('animal', 1)}×")
    if not TRAIN_MODE_MANIFEST:
        logger.info(f"    - score 阈值: {CONF_FILTER}")
    logger.info(f"  阶段一: {YOLO_MODEL} (检测, epochs={EPOCHS})")
    logger.info(f"  阶段二: {Fastsam_MODEL} (零样本分割)")
    logger.info(f"  类别: {TRAIN_CLASSES}")
    logger.info("=" * 60)
    ...
    if not TRAIN_REBUILD_DATASET and dataset_yaml.exists():
        ...
        if TRAIN_REBUILD_DATASET:
            logger.info("REBUILD_DATASET=True，强制重建 train/val 图像、标签和 dataset.yaml")
        if TRAIN_MODE_MANIFEST:
            n_train = convert_split_from_manifest(
                "train",
                TRAIN_MANIFEST_JSON,
                train_set,
                TRAIN_IMAGES_DIR,
        ...
        n_val = convert_split("val", TRAIN_VAL_PRED_JSON, val_set,
                              VAL_IMAGES_DIR, VAL_LABELS_DIR, do_oversample=False) \
                if TRAIN_VAL_PRED_JSON.exists() else 0
        create_dataset_yaml()
    ...
    if best_pt and best_pt.exists() and TRAIN_VAL_PRED_JSON.exists():
        logger.info(f"\n{'─' * 50}\n两阶段 mask mIoU 评估")
        two_stage_eval(best_pt, TRAIN_VAL_PRED_JSON, VAL_LIST)
